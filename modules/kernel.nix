...
in {
  ...
  in {
    mth.kernel.build = pkgs.linuxKernel.manualConfig {
      src = upstreamKernel.src;
      version = upstreamKernel.version;
    ...
    mth.dev.kernel = let
      ...
      '';
      mkReconfigure = name: makePart: let
          kernelNixTarget = "$SOURCE_ROOT/platforms/${config.mth.platform.name}/kernel.nix";
        in pkgs.pkgsBuildBuild.writeShellScriptBin "reconfigure-kernel" ''
          source ${kernelWithNcurses.inputDerivation}
          source $stdenv/setup
          export TMPDIR="/tmp"
          ...
          echo '# To modify kernel configuration, use command `just reconfigure-kernel ${config.mth.platform.name}`' >> ${kernelNixTarget}
          echo "" >> ${kernelNixTarget}
          echo '{' >> ${kernelNixTarget}
          echo "  mth.kernel.config = ${"''"}" >> ${kernelNixTarget}
          cat $KCONFIG_CONFIG >> ${kernelNixTarget}
          echo "  ${"''"};" >> ${kernelNixTarget}
          echo '}' >> ${kernelNixTarget}
          rm -rf $TMPDIR
        '';
